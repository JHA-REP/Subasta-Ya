...
public class UsuariosControlador : ControllerBase
{
    ...
    public async Task<IActionResult> ListadoPujasPorUsuario(
        int usuarioId,
        [FromServices] Aplicacion.CasosDeUso.Pujas.ListarPujasPorUsuario.ListadoPujasPorUsuarioManejador manejador)
    {
        var resultado = await manejador.EjecucionAsync(
            new Aplicacion.CasosDeUso.Pujas.ListarPujasPorUsuario.ListadoPujasPorUsuarioConsulta(usuarioId));
        return Ok(resultado);
    }
}
